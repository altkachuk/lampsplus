...
import com.zugara.atproj.lampsplus.utils.DpPxConverter;
...
public class InvoicePresenter extends BaseFilePresenter {
    ...
    private void loadLamp() {
        if (invoiceIndex < sessionContext.getInvoiceItems().size()) {
            ...
            if (invoiceItem.getLamp().getSource() instanceof File) {
                picasso.load((File) invoiceItem.getLamp().getSource()).resize(DpPxConverter.dpToPixel(context, IMAGE_WIDTH), DpPxConverter.dpToPixel(context,ITEM_HEIGHT)).centerInside().into(target);
            }
        } else {
            drawTotalData();
